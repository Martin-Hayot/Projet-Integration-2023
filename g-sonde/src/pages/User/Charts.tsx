import {
    IonContent, IonSelect, IonSelectOption, IonItem, IonList, IonPage, IonText,
} from '@ionic/react';
...
const Charts: React.FC = () => {
    ...
    useEffect(() => {
        requireAuth(history);
        axios.get(api_link + '/api/aquarium', {
    ...
    const aquariumDiagnostic = (aquarium: string) => {
        ...
        axios.get(api_link + '/api/aquarium/' + aquarium + '/diagnostic', { withCredentials: true })
            .then((response) => {
                if (response.status === 401 || response.status === 400) {
                    window.location.href = "/login?error=unauthorized";
                    return;
    ...
    const diagnosticData = (diagnostic: string) => {
        axios.get(api_link + '/api/aquarium/' + aquariumId + '/diagnostic/' + diagnostic + '/data', { withCredentials: true })
            .then((response) => {
                if (response.status === 401 || response.status === 400) {
                    window.location.href = "/login?error=unauthorized";
                    return;
    ...
    return (
        <IonPage>
            ...
            <IonList>
                <IonItem>
                    <IonSelect placeholder="Selectionner un aquarium" onIonChange={(e) => aquariumDiagnostic(e.detail.value)}>
                        <div slot="label">
